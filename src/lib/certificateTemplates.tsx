...
const renderHeirRows = (heirs: CertificateTemplateContext['heirs']) => {
  ...
  return heirs.map((heir, index) => (
    <tr key={heir.id} className="align-top border-b border-slate-300">
      ...
      <td className="border border-slate-300 px-3 py-3">{heir.relationship}</td>
      <td className="border border-slate-300 px-3 py-3">
        {heir.nidOrBirthRegistration ? heir.nidOrBirthRegistration : '—'}
      </td>
      <td className="border border-slate-300 px-3 py-3">{heir.comment ?? '—'}</td>
    </tr>
  ))
}
        ...
        <div className="certificate-sheet">
          <div className="certificate-paper">
            ...
            <div className="mt-6 text-justify leading-8 text-[17px] text-slate-800">
              এই মর্মে প্রত্যয়ন করা যাচ্ছে যে, <span className="font-bold">{context.person.name}</span>,
              পিতা: <span className="font-bold">{context.person.fatherOrHusbandName}</span>
              {motherName ? <>, মাতা: <span className="font-bold">{motherName}</span></> : null}
              । তিনি অত্র ইউনিয়নের <span className="font-bold">{context.person.ward ?? 0}</span> নং ওয়ার্ড-এর অন্তর্গত{' '}
            ...
            <div className="mt-6 overflow-hidden rounded-md border border-slate-300">
              <table className="w-full border-collapse text-[15px]">
                ...
                <tbody>{renderHeirRows(context.heirs)}</tbody>
              </table>
            </div>
